...
class Rdt_server extends Rdt{
	...
	public boolean rdt_rcv(DatagramSocket serverSocket){//format: seq,checksum
		...
		if(corrupt || !isAck) {
			if(corrupt){
				System.out.println("TRANSFER ERROR: Packet sent from client was corrupted");
			}else if(!isAck){
				System.out.println("TRANSFER ERROR: Packet sent from client was not of the right sequence expected");
			}
...
class Rdt_client extends Rdt{
	...
	public void rdt_send(int seq,DatagramSocket clientSocket,InetAddress IPAddress,int port){//format: seq,checksum
		byte[] sndpkt;
		sndpkt=make_pkt(seq);
	...
	public byte[] rdt_rcv(DatagramSocket clientSocket,InetAddress IPAddress,int port){//format: checksum,seq,data
		...
		if(corrupt || !correctSeq) {
			if(corrupt) System.out.println("TRANSFER ERROR: Packet sent from server was corrupted");
			else if(!correctSeq){
				System.out.println("TRANSFER ERROR: Packet sent from server was not of the right sequence expected");
			}
			
			rdt_send((expectedSeq+1)%2,clientSocket,IPAddress,port);// send nack
			return rdt_rcv(clientSocket,IPAddress,port);//call the function again until data is received correctly
		}else{ //not corrupt and correct seq
			rdt_send(expectedSeq,clientSocket,IPAddress,port);// send ack
			//return data aspect
